...
1. Check the `WriteUserEventRequest` request example in the <walkthrough-editor-select-regex filePath="cloudshell_open/interactive-tutorial/RetailEvents.Samples/WriteUserEventSample.cs" regex="# get write user event request">`RetailEvents.Samples/WriteUserEventSample.cs`</walkthrough-editor-select-regex> file.

1. Run the code sample in the Terminal with the following command:
    ```bash
    dotnet run -- WriteUserEventTutorial
    ```
...
1. Find the <walkthrough-editor-select-regex filePath="cloudshell_open/interactive-tutorial/RetailEvents.Samples/WriteUserEventSample.cs" regex="# TO CHECK THE ERROR HANDLING TRY TO PASS INVALID CATALOG:">comment</walkthrough-editor-select-regex> and uncomment the next line.
...
1. Check the error message:
    ```terminal
    google.api_core.exceptions.NotFound: 404 catalog_id 'InvalidCatalog' not found for project. In most cases, this should be set to 'default_catalog'.
...
<walkthrough-tutorial-card id="retail__retail_api_v2_rejoin_user_events_dotnet" icon="LOGO_NET" title="Rejoin user events tutorial" keepPrevious=true>
